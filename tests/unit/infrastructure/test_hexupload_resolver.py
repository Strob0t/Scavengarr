...
from scavengarr.infrastructure.hoster_resolvers.hexupload import (
    HexuploadResolver,
    _extract_file_id,
)
...
class TestExtractFileId:
    def test_main_domain(self) -> None:
        assert _extract_file_id("https://hexupload.net/abc123def456") == "abc123def456"

    def test_with_filename(self) -> None:
        url = "https://hexupload.net/abc123def456/movie.mkv.html"
        assert _extract_file_id(url) == "abc123def456"

    def test_www_prefix(self) -> None:
        assert _extract_file_id("https://www.hexupload.net/abc123def456") == "abc123def456"

    def test_http_scheme(self) -> None:
        assert _extract_file_id("http://hexupload.net/abc123def456") == "abc123def456"
    ...
    def test_short_id_rejected(self) -> None:
        assert _extract_file_id("https://hexupload.net/abc123") is None
...
class TestHexuploadResolver:
    def test_name(self) -> None:
        client = MagicMock(spec=httpx.AsyncClient)
        assert HexuploadResolver(http_client=client).name == "hexupload"
    ...
    async def test_resolves_valid_file(self) -> None:
        ...
        mock_resp.url = "https://hexupload.net/abc123def456"

        client = AsyncMock(spec=httpx.AsyncClient)
        client.get = AsyncMock(return_value=mock_resp)

        url = "https://hexupload.net/abc123def456"
        result = await HexuploadResolver(http_client=client).resolve(url)
    ...
    async def test_returns_none_for_offline(self) -> None:
        ...
        result = await HexuploadResolver(http_client=client).resolve(
            "https://hexupload.net/abc123def456"
        )
    ...
    async def test_returns_none_on_error_redirect(self) -> None:
        ...
        mock_resp.url = "https://hexupload.net/404"
