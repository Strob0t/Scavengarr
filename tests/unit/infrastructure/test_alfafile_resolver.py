...
from scavengarr.infrastructure.hoster_resolvers.alfafile import (
    AlfafileResolver,
    _extract_file_id,
)
...
class TestExtractFileId:
    ...
    def test_www_prefix(self) -> None:
        url = "https://www.alfafile.net/file/abc123def"
        assert _extract_file_id(url) == "abc123def"

    def test_http_scheme(self) -> None:
        url = "http://alfafile.net/file/abc123def"
        assert _extract_file_id(url) == "abc123def"
    ...
    def test_wrong_path(self) -> None:
        assert _extract_file_id("https://alfafile.net/download/abc123def") is None
...
_VALID_PAGE = """
<html>
<head><title>AlfaFile.net</title></head>
<body>
...
class TestAlfafileResolver:
    def test_name(self) -> None:
        client = MagicMock(spec=httpx.AsyncClient)
        resolver = AlfafileResolver(http_client=client)
        assert resolver.name == "alfafile"
    ...
    async def test_resolves_valid_file(self) -> None:
        ...
        mock_resp.url = "https://alfafile.net/file/abc123def"
    ...
    async def test_returns_none_for_offline(self) -> None:
        ...
        resolver = AlfafileResolver(http_client=client)
        result = await resolver.resolve("https://alfafile.net/file/abc123def")
    ...
    async def test_returns_none_on_error_redirect(self) -> None:
        ...
        mock_resp.url = "https://alfafile.net/404"
